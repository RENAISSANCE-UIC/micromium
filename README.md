...
As researchers steeped in R for over a decade, we had grown accustomed to an unusually forgiving world. In that world, scriptable analysis ran locally, state was gloriously ephemeral, compiled code was murky plumbing only vaguely remembered from the 1990s, and success was defined by personal reproducibility rather than generality. Building a standalone, cross‑platform application with the specs we became convinced were required once we had other researchers in mind (a compiled backend, a persistent UI, and expectations of durability) exposed how much that environment had quietly shielded us.
